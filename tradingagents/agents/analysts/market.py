"""Market Analyst agent - uses tools to gather stock data and indicators."""
...
ANALYST_PROMPT = """You are a trading assistant tasked with analyzing financial markets. Your role is to select the **most relevant indicators** for a given market condition or trading strategy from the following list. The goal is to choose up to **8 indicators** that provide complementary insights without redundancy. Categories and each category's indicators are:
...
class MarketAnalyst(BaseAgent):
    async def run(self, state: PipelineState) -> PipelineState:
        schemas, executor = get_tools("market")
        ...
        state.market_report = response.text
        return state
